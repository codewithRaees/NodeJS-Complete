...
function DirectoryView() {
  const BASE_URL = "http://192.168.71.177:4000";
  const [directoryItems, setDirectoryItems] = useState([]);
  const [progress, setProgress] = useState(0);
  const [newFilename, setNewFilename] = useState("");
   const {"*": direPath} = useParams()
   console.log(direPath)
  async function getDirectoryItems() {
    const response = await fetch(`${BASE_URL}/directory/${direPath}`);
  ...
  return (
    <>
      ...
      {directoryItems.map(({name , isDirectory }, i) => (
        <div key={i}>
          {name}
           {isDirectory &&  <a href={`/${name}?action=open`}>Open</a>}
